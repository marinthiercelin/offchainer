...
abstract contract SecretRequester {
    ...
    function callback(uint256 id, uint128[2] memory f_inputs, uint128 output) public isFromHolder(){
        end(id, f_inputs, output);
    }

    function end(uint256 id, uint128[2] memory f_inputs, uint128 output) internal virtual;
}
...
abstract contract SecretHolder {
    ...
    function requestComputation(uint128[2] memory f_inputs)
        public
        payable
        isFromRequester()
    returns (uint256){
    ...
    function makeComputation(uint256 id, uint128[2] memory f_inputs) internal virtual;
}
...
abstract contract OffChainSecretHolder is SecretHolder {
    ...
    event NewRequest(uint256 id, uint128[2] inputs, uint256 reward);
    event NewAnswer(uint256 id, uint128[2] inputs, uint128 output);

    function makeComputation(uint256 id, uint128[2] memory f_inputs) internal override{
...
contract ZokratesHolder is OffChainSecretHolder {
    ...
    function verifyProof(uint128[2] memory f_inputs, uint128 output, Proof memory proof) internal returns (bool){
        uint256[2+2+1] memory verifier_inputs;
        for(uint i = 0; i < 2; i++){
            verifier_inputs[i] = f_inputs[i];
        }
        for(uint i = 0; i < 2; i++){
            verifier_inputs[2+i] = commitment[i];
        }
        verifier_inputs[2+2] = output;
