...
contract __PROJECT_NAME__Requester is SecretRequester {
    ...
    event End(uint256 id, uint128[__NB_PUB_INPUTS__] inputs, uint256 result);

    // You can change the signature and arguments of this function
    function start(uint128[__NB_PUB_INPUTS__] memory user_inputs) public returns (uint256) {
        // You can change the way f_inputs are computed
        uint128[__NB_PUB_INPUTS__] memory f_inputs = user_inputs;
    ...
    function end(uint256 id, uint128[__NB_PUB_INPUTS__] memory f_inputs, uint128 output) internal override{
